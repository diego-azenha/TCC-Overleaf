...
\usepackage{tikz}
\usetikzlibrary{arrows.meta}      % setas Stealth
\usetikzlibrary{shapes.geometric} % diamond, ellipse, trapezium
\usetikzlibrary{shapes.symbols}   % signal (hexágono do decoder)
\usetikzlibrary{shapes.misc}      % chamfered rectangle, rounded rectangle
\usetikzlibrary{positioning}      % above=of, right=of
\usetikzlibrary{calc}             % ($(node) + (x,y)$) no decoder
\usepackage{bm}                   % \bm{} no decoder e preditor
...
% Título
\titulo{ Factor Investing sob a Perspectiva do \\ Machine Learning Probabilístico: \\}
% Subtítulo]
\subtitulo{Aplicação do FactorVAE no \\ Mercado de Ações Brasileiro}
% Autor (nome completo)
\autor{Diego Fullin Azenha}
% Ano com 4 dígitos
...
\orientador[Orientador:]{Ruy Monteiro Ribeiro}
% Tipo de trabalho (TCC  / Dissertação / Tese)
\tipotrabalho{Trabalho de Conclusão de Curso}
% Palavras-chaves (importante: separar por vírgulas)
\palavraschaves{modelos de fatores, autoencoder variacional, retornos de ações, mercado brasileiro, aprendizado de máquina, inferência variacional}
% Palavras-chaves em inglês (importante: separar por vírgulas)
\keywords{factor models, variational autoencoder, stock returns, Brazilian market, machine learning, variational inference}
% Professores convidados (importante: separar por vírgulas)
\professores{Bernardo Ricca}

% Preâmbulo
\preambulo{Trabalho de Conclusão de Curso apresentado ao Curso de Graduação em Economia do Insper Instituto de Ensino e Pesquisa como requisito parcial para a obtenção do título de Bacharel em Economia.}
...
\include{2-textuais/02-revisao}
\include{2-textuais/03-metodologia}
\include{2-textuais/04-resultados}
\include{2-textuais/05-conclusao}
